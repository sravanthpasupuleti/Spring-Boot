		...
		try (Scanner scanner = new Scanner(System.in)) {
			do {
				...
				case 2:
					...
					break;
				case 3:
					String firstname = InputUtil.acceptOwnerFirstNameToOperate(scanner);
					List<OwnerDTO> ownerDTOList = ownerService.findOwnerWithFirstName(firstname);
					System.out.println(String.format("Found %s owners with same firstname",ownerDTOList.size()));
					System.out.println(ownerDTOList);
					break;
				case 4:
					petId = InputUtil.acceptPetIdToOperate(scanner);
					ownerDTO = ownerService.findOwnerWithPetId(petId);
